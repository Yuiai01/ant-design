...
function getMessageGlobalConfig() {
  const {
...
const GlobalHolder = React.forwardRef<GlobalHolderRef, {}>((_, ref) => {
  const [messageConfig, setMessageConfig] = React.useState<ConfigOptions>(getMessageGlobalConfig);
  ...
  const sync = () => {
    setMessageConfig(getMessageGlobalConfig);
  };
...
export const useAppMessage = (independent: boolean) =>
  useInternalMessage(independent ? undefined : getMessageGlobalConfig());
